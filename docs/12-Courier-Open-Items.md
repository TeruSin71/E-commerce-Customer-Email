...
| 2 | Where does the carrier contract/account number live in SAP? | XK03 on one carrier vendor: check LFB1-EIKTO (company-code data), Z-fields, or ME33K outline agreements. If nowhere — create Z-table + SM30 view | Task 1.6 (`ZI_CarrierContract` source) | ✅ CLOSED | **Answered by Teru (2026-07-17): the contract account is PROVIDED INTO the app** — maintained as app config (`CarrierAccounts` rows, SysAdmin config scope), NOT read from SAP. Placeholder rows seeded per courier (`db/data/`), `active=false` until the real account ref is entered. The `ZI_CarrierContract` ECC CDS view is DROPPED from 1.2 scope; doc 08 §4.1 updated in the same commit. Fail-closed router rule unchanged: no active contract row → no quote, ever. |
...
| 4 | Is each plant's T001W→ADRC address the physical dispatch dock, or a registered office? | OX10 / ADRC per plant, confirm with warehouse | Task 1.2 / Phase 3 per region. Wrong = wrong rates + wrong pickup | ✅ CLOSED | **Answered by Teru (2026-07-17): the dispatch address comes from an SPRO-style CONFIGURATION table on the ECC side, exposed via a CDS view** (not the T001W→ADRC master-data join). The dock-vs-office ambiguity disappears — the config table holds the address dispatch wants. `ZI_PlantAddress` in doc 08 §4.1 redefined accordingly (same field shape, new source); built at task 1.2. Doc 08 updated in the same commit. |
...
| 6 | Does e-commerce already send shipping/tracking emails? | Ask e-commerce team | Task 1.13 content/ownership — avoid duplicate customer comms. If e-comm owns comms, consider pushing tracking to e-comm instead of emailing directly | ✅ CLOSED | **Answered by Teru (2026-07-17): the COURIER APP owns the customer tracking email** — e-commerce does not send one. 1.13 builds the direct Graph send as specced. Until real ECC lands (1.2), the synthetic fixture carries `terulin.sinulingga@gallagher.com` as every ship-to email, so all dev/test sends go to Teru; the REAL recipient is the delivery's ADR6 email (per Open Item #3's join) flowing through `ship_to_email` — the logic reads it from the delivery payload, never from config. |
