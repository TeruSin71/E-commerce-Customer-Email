...
| 1.17 | Work Zone content — 4 tiles wired to role collections; SoD verified | 1.15, 1.16 | 🤖 | 🟡 *(1.17a 2026-07-18: both tiles live + role-gated; **Lookup verified working on a real user**; **Dashboard data-load RESOLVED 2026-07-18** — root cause was the Work Zone content provider pinning a stale app version; provider re-sync (Channel Manager → 🔄) surfaced the correct v0.0.3 fixes. Deploy checklist now: version bump → `cf deploy` → provider re-sync → incognito F12 — see task-log session-5 entry. Remaining: dispatch/config tiles gated on 0.1/1.15)* | |
