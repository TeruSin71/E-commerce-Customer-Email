...
- **Codebase questions — pick the right tool.** For **JS/service-logic** questions ("where is X handled", call flow, what a handler does), reach for the `graphify` skill first — it queries a prebuilt knowledge graph instead of re-reading files. **But graphify does NOT parse `.cds` (no CDS grammar)**, so for the **CDS layer — the data model, entities/associations, OData service exposure, `@restrict`/`@requires` security, and Fiori annotations** — use `cds-mcp` (`search_model`) and read the `.cds` sources directly; graphify is blind there and will return a confident but incomplete answer. Build once with `graphify ./srv --code-only` (scope it to source dirs — never the repo root or `$HOME`). `graphify-out/` is gitignored and must stay uncommitted; the graph can capture repo content (non-negotiable #3).
