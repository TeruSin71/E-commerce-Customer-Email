...
- **Codebase questions default to `graphify`.** For "where is X", architecture, or file-relationship questions, reach for the `graphify` skill first — it queries a prebuilt knowledge graph instead of re-reading files. Build once with `graphify ./docs ./srv db app` (scope it to source dirs — never the repo root or `$HOME`). `graphify-out/` is gitignored and must stay uncommitted; the graph can capture repo content (non-negotiable #3).
